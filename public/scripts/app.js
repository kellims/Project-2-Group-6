...
searchForm.addEventListener("submit", (e) => { //we attache en event listener to the submit action on the searchform
  ...
  fetchAPI(); // is called to perform the API call and display the results                               fetch method Source Mmdn web docs Fetch API
  //console.log(searchQuery)
});

// Source API https://developer.edamam.com/edamam-docs-recipe-api-v1
async function fetchAPI () {
    const baseURL = `https://api.edamam.com/search?q=${searchQuery}&app_id=${APP_ID}&app_key=${APP_key}&to=26`; //use backtick to have dynamic javascript in this string // by adding "&to=26" I can see until 26 results //part2 change pasta by a javascript expression "${searchQuery}" to do the queries
    const response = await fetch(baseURL);
    const data = await response.json(); // it will convert our response into json and we will have jason into the data
